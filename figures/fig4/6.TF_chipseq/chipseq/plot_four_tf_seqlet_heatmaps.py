...
One row per seqlet occurrence, including duplicate genomic loci. ±1000 bp,
motif-oriented, sorted by central ±100 bp mean signal. Raw bigWig values:
    ...
    with h5py.File(MOD/'count_tfmodisco_patterns.h5') as h:
        for (tf,pat,expected,filename),dest in zip(SPECS,paths):
            ...
            with warnings.catch_warnings():
                ...
                strength=np.nanmean(mat[:,a.flank-100:a.flank+101],axis=1)
            ...
            ax.set_xlabel('From seqlet center (bp)'); ax.set_ylabel('Seqlets (sorted)')
            ...
            fig.subplots_adjust(left=.17,right=.86,bottom=.18,top=.79)
            fig.canvas.draw()
