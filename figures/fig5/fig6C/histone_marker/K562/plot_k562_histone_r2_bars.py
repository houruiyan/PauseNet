#!/usr/bin/env python3
"""Plot K562 histone-marker adjusted-R2 comparisons.

For each histone marker, plot:

1. K562 histone only
2. PauseNet + K562 histone (matched cell type)
3. PauseNet + HeLaS3 histone (mismatched cell type)

The gray and red dashed lines show PauseNet only and PauseNet plus all
K562 histone markers, respectively. Their values are read directly from
the batch-analysis summaries.
"""
...
DEFAULT_RESULT_DIR = SCRIPT_DIR / "k562_histone_r2_results"
DEFAULT_INPUT = DEFAULT_RESULT_DIR / "k562_histone_r2_summary.tsv"
DEFAULT_ALL_SUMMARY = (
    DEFAULT_RESULT_DIR / "k562_all_histones_r2_summary.tsv"
)
DEFAULT_OUTPUT = DEFAULT_RESULT_DIR / "k562_histone_r2_grouped_bars.pdf"

HISTONE_ONLY_COLOR = "#f9c00c"
PAUSENET_K562_COLOR = "#00b9f1"
PAUSENET_HELAS3_COLOR = "#f9320c"
...
ALL_HISTONES_COLUMN = "PauseNet_plus_all_K562_histones_R2_adj"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--title",
        default="K562",
        help="Figure title (default: K562); use an empty string to hide.",
    )
    parser.add_argument(
        "--show-values",
        action="store_true",
...
def main() -> None:
    ...
    print(f"PauseNet + all K562 histones: {pausenet_all_histones:.6f}")
