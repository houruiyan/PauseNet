...
readonly DEFAULT_NPZ="/mnt/HDD8TB/houruiyan/pausing_site/2_train_model/evaluate/helas3_netseq/test_profiles.npz"
readonly DEFAULT_TRACKS="${SCRIPT_DIR}/histone_tracks.tsv"
readonly DEFAULT_RESULT_DIR="${SCRIPT_DIR}/helas3_histone_r2_results"

usage() {
    cat <<EOF
Usage:
  bash $(basename "$0") [HELAS3_TEST_NPZ] [HISTONE_TRACKS_TSV] [RESULT_DIR]

Arguments are optional. Defaults:
  HELAS3_TEST_NPZ    ${DEFAULT_NPZ}
...
for i in "${!MARKERS[@]}"; do
    ...
    run_one \
        "${marker}" \
        "${helas3_bigwig}" \
        "${RESULT_DIR}/helas3_${slug}_helas3.tsv"
    run_one \
        "${marker}" \
        "${k562_bigwig}" \
        "${RESULT_DIR}/helas3_${slug}_k562.tsv"
done
...
if (( ${#MARKERS[@]} >= 2 )); then
    ...
    "${PYTHON_BIN}" "${CALCULATE_SCRIPT}" \
        ...
        --only-all-tfs \
        --threads "${THREADS}" \
        --output-tsv "${RESULT_DIR}/helas3_all_helas3_histones.tsv"
    ...
    "${PYTHON_BIN}" "${CALCULATE_SCRIPT}" \
        ...
        --only-all-tfs \
        --threads "${THREADS}" \
        --output-tsv "${RESULT_DIR}/helas3_all_k562_histones.tsv"
fi
...
summary_path = result_dir / "helas3_histone_r2_summary.tsv"
report_path = result_dir / "helas3_histone_r2_report.txt"
...
with (
    ...
    for track in tracks:
        ...
        baseline = 0.5 * (helas3["model_only"] + k562["model_only"])
        matched_advantage = (
            helas3["model_plus_tf"] - k562["model_plus_tf"]
        )
...
if len(tracks) >= 2:
    helas3_all = read_values(result_dir / "helas3_all_helas3_histones.tsv")
    k562_all = read_values(result_dir / "helas3_all_k562_histones.tsv")
    all_summary = result_dir / "helas3_all_histones_r2_summary.tsv"
    with all_summary.open("w", encoding="utf-8", newline="") as handle:
        ...
        baseline = 0.5 * (
            helas3_all["model_only"] + k562_all["model_only"]
        )
        matched_advantage = (
...
echo "Completed:"
echo "  Summary: ${RESULT_DIR}/helas3_histone_r2_summary.tsv"
echo "  Report:  ${RESULT_DIR}/helas3_histone_r2_report.txt"
if (( ${#MARKERS[@]} >= 2 )); then
    echo "  All:     ${RESULT_DIR}/helas3_all_histones_r2_summary.tsv"
fi
