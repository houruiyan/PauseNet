#!/usr/bin/env python3
"""Profile-only raincloud; reuse original G4Hunter scores.
...
p.add_argument('--input',default='/mnt/HDD8TB/houruiyan/pausing_site/4_plot_figure/fig2/G4/boxplot/cluster1_g4hunter_scores.tsv')
p.add_argument('--output',default=str(Path(__file__).resolve().parent/'profile_g4hunter_raincloud.pdf'))
a=p.parse_args()
...
values=[d.loc[d.pattern_number.eq(k),'g4hunter_score'].to_numpy() for k in patterns]
...
ax.set_xlabel('G4Hunter score');ax.set_title('Profile patterns',fontsize=14,pad=12)
ax.set_xlim(min(-2.6,min(v.min() for v in values)-.2),max(3.6,max(v.max() for v in values)+.2))
ax.axvline(1.2,color='#888888',lw=.8,ls='--',zorder=0)
ax.set_xticks([x for x in range(-2,4) if ax.get_xlim()[0]<=x<=ax.get_xlim()[1]])
